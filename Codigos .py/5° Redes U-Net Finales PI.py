...
def cargar_carpetas(carpeta, carpeta_estruc, carpeta_dosis):
    ...
    dosis = sorted(dosis) # ordenar para que las posiciones de las duplas dosis-órganos coincidan entre las dos carpetas
...
estruc, dosis = cargar_carpetas('DatosPI/EntrenamientoPI', 'Estructuras', 'Dosis') 
estruc #Desde el ejemplo 72 se utilizan para validación 
...
def testdata():
    ...
    for i in range(len(dosis)):
        lista.append(str(dosis[i]).split("/")[-1])
...
def fitB(model, dataloader, epochs=10, lr=1e-4):
    ...
    for epoch in range(1, epochs+1):
        ...
        if np.mean(test_loss) < best_valid_loss:
            data_str = f"El loss de validación mejoró de {best_valid_loss:.5f} a {np.mean(test_loss):.5f}. Guardando checkpoint: {checkpoint_path}"
            print(data_str)
...
def cargar_ejem (dir_est, dir_targets, dirpredUB, dirpredUM, dirpredRU, i, graph=True):
    ...
    if graph==True:
        ...
        sns.set_style("dark")
        ax1.imshow(target[:,:,24],cmap="magma")
        ax2.imshow(pred1[:,:,24],cmap="magma")
        ax3.imshow(pred2[:,:,24],cmap="magma")
        ax4.imshow(pred3[:,:,24],cmap="magma")
...
est, target, pred1, pred2, pred3 = cargar_ejem('DatosPI/TesteoPI/Estructuras',
                                               'DatosPI/TesteoPI/Dosis',
                                               'DatosPI/TesteoPI/UB',
                                               'DatosPI/TesteoPI/UM',
                                               'DatosPI/TesteoPI/RU',
                                                i=16)
...
ejemplos()# Con esta funcion se comienzan a generar las graficas de todos los ejemplos
...
def Dice_final():
    ...
    for i in range(30):
        est, true, pred1, pred2, pred3 = cargar_ejem('DatosPI/TesteoPI/Estructuras',
                                               'DatosPI/TesteoPI/Dosis',
                                               'DatosPI/TesteoPI/UB',
                                               'DatosPI/TesteoPI/UM',
                                               'DatosPI/TesteoPI/RU',
