...
def flip(path):
    ...
    for i,f in sorted(enumerate(lista_np)):
        ...
        np.save(f'DataSetPI/{carpeta}_flip/{nombre}.flip',est_flip)
...
est = np.load('DatasetPI2/Estructuras/estructura14.npy')
dos = np.load('DatasetPI2/Dosis/dosis14.npy')
est_flip = np.load('DataSetPI2/Estructuras_flip/estructura14.flip.npy')
dos_flip = np.load('DataSetPI2/Dosis_flip/dosis14.flip.npy')
...
#Primero cambiar la carpeta donde se van a guardar 
...
for i in range(len(estruc)):
    ...
    dos = np.load(dosis[i])
    estf = np.load(estruc_flip[i])
    dosf = np.load(dosis_flip[i])
    fig, (ax1, ax2, ax3, ax4) = plt.subplots(1, 4, figsize=(30,10))
    j=24
    ax1.imshow(dos[:,:,j],cmap="magma")
    ax2.imshow(dosf[:,:,j],cmap="magma")
    ax3.imshow(est[:,:,j],cmap="magma")
    ax4.imshow(estf[:,:,j],cmap="magma")
    ax1.set_title(dosis[i])
    ax2.set_title(dosis_flip[i])
    ax3.set_title(estruc[i])
    ax4.set_title(estruc_flip[i])
    plt.show()
...
# create_dir ("DatosPI/EntrenamientoPI/Estructuras")
# create_dir ("DatosPI/EntrenamientoPI/Dosis")

# for i in range(len(estruc)):
#     shutil.copy(estruc[i] , "DatosPI/EntrenamientoPI/Estructuras")
#     shutil.copy(estruc_flip[i] , "DatosPI/EntrenamientoPI/Estructuras")
#     shutil.copy(dosis[i] , "DatosPI/EntrenamientoPI/Dosis")
#     shutil.copy(dosis_flip[i] , "DatosPI/EntrenamientoPI/Dosis")


# In[33]:


estruc, dosis = cargar_carpetas('DatosPI/EntrenamientoPI', 'Estructuras', 'Dosis')
...
# create_dir ("DatosPI/TesteoPI/Estructuras")
# create_dir ("DatosPI/TesteoPI/Dosis")

# for i in range(len(estruc)):
#     shutil.copy(estruc[i] , "DatosPI/TesteoPI/Estructuras")
#     shutil.copy(estruc_flip[i] , "DatosPI/TesteoPI/Estructuras")
#     shutil.copy(dosis[i] , "DatosPI/TesteoPI/Dosis")
#     shutil.copy(dosis_flip[i] , "DatosPI/TesteoPI/Dosis")


# In[39]:


estruc, dosis = cargar_carpetas('DatosPI/TesteoPI', 'Estructuras', 'Dosis')
